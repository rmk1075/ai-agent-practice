...
describe('streamConversationMessage', () => {
  ...
  it('sends FormData (not JSON)', async () => {
    ...
    await streamConversationMessage(1, 'hello', undefined, vi.fn(), vi.fn(), vi.fn())
  ...
  it('appends file to FormData when provided', async () => {
    ...
    await streamConversationMessage(1, 'hello', file, vi.fn(), vi.fn(), vi.fn())
    const [, options] = mockFetch.mock.calls[0]
  ...
  it('calls onError with server error message on 4xx', async () => {
    ...
    const onError = vi.fn()
    await streamConversationMessage(1, 'hello', undefined, vi.fn(), vi.fn(), onError)
  ...
  it('calls onDone when [DONE] is received', async () => {
    mockFetch.mockResolvedValue({ ok: true, body: makeDoneStream() })
    const onDone = vi.fn()
    await streamConversationMessage(1, 'hello', undefined, vi.fn(), onDone, vi.fn())
  ...
  it('calls onToken with token data', async () => {
    ...
    const onToken = vi.fn()
    await streamConversationMessage(1, 'prompt', undefined, onToken, vi.fn(), vi.fn())
  ...
  it('does not append content field when content is empty string', async () => {
    ...
    await streamConversationMessage(1, '', file, vi.fn(), vi.fn(), vi.fn())
    const [, options] = mockFetch.mock.calls[0]
